...
- (void)pushDatabaseBackInPool:(ZGDatabase*)db;
- (ZGDatabase*)db;

@end


@implementation ZGDatabasePool
...
+ (instancetype)databasePoolWithURL:(NSURL *)url {
    return ZGDBReturnAutoreleased([[self alloc] initWithPath:url.path]);
}

+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
    return ZGDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}

+ (instancetype)databasePoolWithURL:(NSURL *)url flags:(int)openFlags {
    return ZGDBReturnAutoreleased([[self alloc] initWithPath:url.path flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags vfs:(NSString *)vfsName {
    ...
    if (self != nil) {
        ...
        _databaseInPool     = ZGDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = ZGDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    ZGDBRelease(_path);
    ZGDBRelease(_databaseInPool);
    ZGDBRelease(_databaseOutPool);
    ZGDBRelease(_vfsName);
    
    if (_lockQueue) {
        ZGDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(ZGDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The ZGDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
