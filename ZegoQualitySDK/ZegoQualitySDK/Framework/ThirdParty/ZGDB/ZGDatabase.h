...
#import "ZGResultSet.h"
#import "ZGDatabasePool.h"

NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define ZGDBAutorelease(__v) ([__v autorelease]);
    #define ZGDBReturnAutoreleased ZGDBAutorelease

    #define ZGDBRetain(__v) ([__v retain]);
    #define ZGDBReturnRetained ZGDBRetain

    #define ZGDBRelease(__v) ([__v release]);

    #define ZGDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define ZGDBAutorelease(__v)
    #define ZGDBReturnAutoreleased(__v) (__v)

    #define ZGDBRetain(__v)
    #define ZGDBReturnRetained(__v) (__v)

    #define ZGDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define ZGDBDispatchQueueRelease(__v)
    #else
        #define ZGDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
 ...
 */
typedef int(^ZGDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

/**
 Enumeration used in checkpoint methods.
 */
typedef NS_ENUM(int, ZGDBCheckpointMode) {
    ZGDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    ZGDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    ZGDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    ZGDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};
...
/** A SQLite ([https://sqlite.org/](https://sqlite.org/)) Objective-C wrapper.
 ...
 The three main classes in ZGDB are:

 - @c ZGDatabase - Represents a single SQLite database.  Used for executing SQL statements.

 - @c ZGResultSet - Represents the results of executing a query on an @c ZGDatabase .

 - @c ZGDatabaseQueue - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 See also

 - @c ZGDatabasePool - A pool of @c ZGDatabase objects

 - @c ZGStatement - A wrapper for @c sqlite_stmt

 External links

 - [ZGDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](https://sqlite.org/)
 - [ZGDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](https://sqlite.org/faq.html)

 @warning Do not instantiate a single @c ZGDatabase  object and use it across multiple threads. Instead, use @c ZGDatabaseQueue .

 */

@interface ZGDatabase : NSObject
...
/** Create a @c ZGDatabase  object.
 
 An @c ZGDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. An zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c ZGDatabase  connection is closed.

 3. @c nil .  An in-memory database is created.  This database will be destroyed with the @c ZGDatabase  connection is closed.
...
ZGDatabase *db = [ZGDatabase databaseWithPath:fileURL.path];
@endcode
 ...
 @return @c ZGDatabase  object if successful; @c nil  if failure.
...
/** Create a @c ZGDatabase  object.
 ...
 2. @c nil .  An in-memory database is created.  This database will be destroyed with the @c ZGDatabase  connection is closed.
...
NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
ZGDatabase *db = [ZGDatabase databaseWithURL:fileURL];
@endcode
...
/** Initialize a @c ZGDatabase  object.
 
 An @c ZGDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. A zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c ZGDatabase  connection is closed.
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 ZGDatabase *db = [[ZGDatabase alloc] initWithPath:fileURL.path];
 @endcode
...
/** Initialize a @c ZGDatabase  object.
 ...
 An @c ZGDatabase  is created with a local file URL to a SQLite database file.  This path can be one of these three:
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 ZGDatabase *db = [[ZGDatabase alloc] initWithURL:fileURL];
 @endcode
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) ZGDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an @c ZGResultSet  object if successful, and @c nil  upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the @c lastErrorMessage  and @c lastErrorMessage  methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[ZGResultSet next]>`) from one record to the other.
 ...
 @return A @c ZGResultSet  for the result set upon success; @c nil  upon failure. If failed, you can call @c lastError , @c lastErrorCode , or @c lastErrorMessage  for diagnostic information regarding the failure.
 
 @see ZGResultSet
 @see [`ZGResultSet next`](<[ZGResultSet next]>)
 @see [`sqlite3_bind`](https://sqlite.org/c3ref/bind_blob.html)
...
- (ZGResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (ZGResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (ZGResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> ZGResultSet!`
...
- (ZGResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (ZGResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (ZGResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;

/// Prepare SQL statement.
///
/// @param sql SQL statement to prepare, generally with `?` placeholders.

- (ZGResultSet *)prepare:(NSString *)sql;
 ...
 */
- (BOOL)checkpoint:(ZGDBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return @c YES on success, otherwise @c NO .
 */
- (BOOL)checkpoint:(ZGDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return @c YES on success, otherwise @c NO .
 */
- (BOOL)checkpoint:(ZGDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
/// The ZGDB version number as a string in the form of @c "2.7.8" .
///
/// If you want to compare version number strings, you can use NSNumericSearch option:
///
/// @code
/// NSComparisonResult result = [[ZGDatabase ZGDBUserVersion] compare:@"2.11.0" options:NSNumericSearch];
/// @endcode
///
/// @returns The version number string.

+ (NSString*)ZGDBUserVersion;

/** The ZGDB version

 This returns the ZGDB as hexadecimal value, e.g., @c 0x0243 for version 2.4.3.
 ...
 @warning This routine will not work if any component of the version number exceeds 15.
       ...
       For this reason, we would recommend using @c ZGDBUserVersion  and with @c NSNumericSearch  option, e.g.
...
+ (SInt32)ZGDBVersion __deprecated_msg("Use ZGDBUserVersion instead");
...
ZGResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
@code
myDB.dateFormat = [ZGDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
@endcode
 ...
 @warning Note that @c NSDateFormatter  is not thread-safe, so the formatter generated by this method should be assigned to only one ZGDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using @c ZGDatabase:storeableDateFormat .
 ...
 @warning Note there is no direct getter for the @c NSDateFormatter , and you should not use the formatter you pass to ZGDB for other purposes, as @c NSDateFormatter  is not thread-safe.
 */
...
/** Objective-C wrapper for @c sqlite3_stmt
 ...
 This is a wrapper for a SQLite @c sqlite3_stmt . Generally when using ZGDB you will not need to interact directly with `ZGStatement`, but rather with @c ZGDatabase  and @c ZGResultSet  only.
...
@interface ZGStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
