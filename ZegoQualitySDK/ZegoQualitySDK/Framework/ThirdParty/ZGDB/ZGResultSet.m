...
+ (instancetype)resultSetWithStatement:(ZGStatement *)statement usingParentDatabase:(ZGDatabase*)aDB shouldAutoClose:(BOOL)shouldAutoClose {
    ZGResultSet *rs = [[ZGResultSet alloc] init];
    ...
    return ZGDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    ZGDBRelease(_columnNameToIndexMap);
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"ZGDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
